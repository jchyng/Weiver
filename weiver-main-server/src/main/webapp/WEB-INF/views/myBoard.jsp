  ...
  <main class="max-w-4xl mx-auto px-4 pt-24 space-y-8">
    ...
    <div class="flex items-end justify-between">
      <div>
        ...
        <h1 class="text-3xl font-bold mt-1">내가 쓴 글</h1>
      </div>
    ...
    <div class="flex items-center gap-2">
      <button id="btn-date" onclick="sortTypeChange('작성', this)" class="sort-btn active-sort px-5 py-2 rounded-full text-sm font-bold transition-all">
        최신순
      </button>
      <button id="btn-like" onclick="sortTypeChange('추천', this)" class="sort-btn bg-stage-surface hover:bg-white/10 px-5 py-2 rounded-full text-sm font-bold transition-all">
        추천순
      </button>
    </div>
    ...
    <div id="sortDate" class="space-y-3">
      ...
      <c:if test="${empty postListTime}">
        <div class="py-20 text-center space-y-4 bg-stage-surface/30 rounded-3xl border border-dashed border-white/20">
          <i class="bi bi-journal-text text-5xl text-stage-text-sub"></i>
          <p class="text-stage-text-sub font-medium">아직 작성한 글이 없습니다.</p>
        </div>
      </c:if>
    </div>
