  ...
  <main class="max-w-4xl mx-auto px-4 pt-24 space-y-8">
    ...
    <div class="flex items-end justify-between">
      <div>
        ...
        <h1 class="text-3xl font-bold mt-1">내가 쓴 댓글</h1>
      </div>
      <span class="text-stage-text-sub text-sm font-bold">총 <span class="text-stage-text">${replyCount}</span>개</span>
    </div>
    ...
    <div id="sortDate" class="space-y-3">
      <c:forEach var="reply" items="${replyListTime}">
        <a href="${baseURL}/community/${reply.postId}" class="block group">
          <div class="bg-stage-surface/30 border border-white/10 hover:border-white/30 p-5 rounded-2xl transition-all spotlight-glow">
            <div class="flex items-start justify-between gap-4">
              <div class="flex-1 space-y-1.5">
                <p class="text-[10px] text-stage-text-sub font-bold">${reply.createdTime}</p>
                <p class="text-sm leading-relaxed line-clamp-2 group-hover:text-stage-secondary transition-colors">${reply.content}</p>
              </div>
              <div class="flex items-center gap-1.5 shrink-0 text-stage-text-sub">
                <i class="bi bi-heart-fill text-stage-primary/80"></i>
                <span class="font-bold text-sm">${reply.countLikes}</span>
              </div>
            </div>
          </div>
        </a>
      </c:forEach>
      <c:if test="${empty replyListTime}">
        <div class="py-20 text-center space-y-4 bg-stage-surface/30 rounded-3xl border border-dashed border-white/20">
          <i class="bi bi-chat-left-dots text-5xl text-stage-text-sub"></i>
          <p class="text-stage-text-sub font-medium">아직 작성한 댓글이 없습니다.</p>
        </div>
      </c:if>
    </div>

    <!-- Comment List - Like Sort -->
          ...
          <div class="bg-stage-surface/30 border border-white/10 hover:border-white/30 p-5 rounded-2xl transition-all spotlight-glow">
            <div class="flex items-start justify-between gap-4">
              ...
              <div class="flex items-center gap-1.5 shrink-0">
                ...
                <span class="font-bold text-sm text-stage-text">${reply.countLikes}</span>
              </div>
            </div>
          </div>
        </a>
      </c:forEach>
      <c:if test="${empty replyListLike}">
        <div class="py-20 text-center space-y-4 bg-stage-surface/30 rounded-3xl border border-dashed border-white/20">
          <i class="bi bi-chat-left-dots text-5xl text-stage-text-sub"></i>
          <p class="text-stage-text-sub font-medium">아직 작성한 댓글이 없습니다.</p>
        </div>
