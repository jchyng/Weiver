...
public interface SubscribeRepository extends JpaRepository<Subscribe, Long>{
	 ...
	 */
	@Query("SELECT s.musicalId.id as id, s.musicalId.title as title, s.musicalId.posterImage as posterImage FROM Subscribe s WHERE s.type = '봤어요' GROUP BY s.musicalId.id, s.musicalId.title, s.musicalId.posterImage ORDER BY COUNT(s.musicalId) DESC")
	...
	@Query("SELECT s FROM Subscribe s WHERE s.userId.id = ?1 AND s.musicalId.id = ?2 AND s.type = ?3")
	Subscribe getSubscribeByUserIdAndMusicalIdAndType(String userId, String musicalId, String type);
